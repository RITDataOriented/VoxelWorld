...
// [TO DO] figure out permutation and random arrays and if they
// can be parallelized
// currently seems difficult given that the algorithm itself
// does not always use the same number of random numbers
// and a static random can't be used in jobified code
// (so passing a random number array into the alg might be the wrong size)
// currently keeping the alg as serial 
public class NoiseAlgorithm
{
    ...
    private NativeArray<int> permutation = new NativeArray<int>(PERM_SIZE + PERM_SIZE + 2, Allocator.Persistent);
    private NativeArray<float> randomArray = new NativeArray<float>((PERM_SIZE + PERM_SIZE + 2) * DIMENSIONS, Allocator.Persistent);
    private NativeArray<float> noise;
    
    // terrain chunk related variables
    public int width, height, depth, maxHeight;
    ...
    public void InitializeBlocks(int w, int h, int d, int mh, int seaLvl)
    {
        ...
        seaLevel = seaLvl;
        noise = new NativeArray<float>(width*depth, Allocator.Persistent);
...
public struct CreateBlocks : IJobParallelFor
{
    ...
    [ReadOnly] public int width, height, depth, maxHeight;
    [ReadOnly] public int seaLevel;
    [ReadOnly] public NativeArray<float> noise;
    public NativeArray<int> blockIndices;
    private enum BlockTypes : int { Bedrock = 0, Dirt, Grass, Water, Air };
    ...
    public void Execute(int index)
    {
        ...
        int expectedHeight = (int)(noise[x * width + z] * maxHeight);
        if (y < 3)
        {
            blockIndices[index] = (int)BlockTypes.Bedrock;
        }
        else if (y < expectedHeight)
        {
            // make this block dirt
            blockIndices[index] = (int)BlockTypes.Dirt;

        }
        else if (expectedHeight == y)
        {
            blockIndices[index] = (int)BlockTypes.Grass;
        }

        else if (seaLevel >= y)
        {
            blockIndices[index] = (int)BlockTypes.Water;
        }
        else
        {
            // make this block air
            blockIndices[index] = (int)BlockTypes.Air;
        }
...
public struct NormalizePerlin : IJobParallelFor
{
    ...
    public void Execute(int index)
    {
        ...
        heightMap[index] = normaliseMin + normalisedHeight;
    }
